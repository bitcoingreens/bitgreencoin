...
#if defined(NDEBUG)
# error "Bitgreencoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcd6391ba0ceb9975c9636a548bf996940e17ae77743999b05ebad16dc7f9c351");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitgreencoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitgreencoin Signed Message:\n";
...
{
    // Bitgreencoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Bitgreencoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Bitgreencoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Bitgreencoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitgreencoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitgreencoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbdc48a97ad309113127abc16cf73aff97298b3a63305226560f141734cabcbeb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Real Madrid 26/Mai/2021 : Modric resign a new  contract at 25/Mai/2021 for one year";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 12.5 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041c0b90ac2e25a0895c0e9b1e8ac9ec0f9a1bc4e38dc8e98fe47bc9b089edd9a17fcaf3db2019a11473270fc9ac81684b242c825de6a1cf0dfed15ff8da086cc9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1622062156;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084664215;

        if (fTestNet)
        {
            block.nTime    = 1622062084;
            block.nNonce   = 388263534;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb2cd8bab0a15fa933b658ccfccee3a13afcb06f4f3e3010d29d8a50a883313e9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xbb, 0xd1 }; // Bitgreencoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitgreencoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitgreencoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitgreencoinMiner(CWallet *pwallet)
{
    printf("BitgreencoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitgreencoin-miner");
    ...
    try { loop {
        ...
        printf("Running BitgreencoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitgreencoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitgreencoinMiner, pwallet));
}
